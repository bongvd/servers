...
const mathUtils = {
        ...
        for (var i = 0; i < content.length; i++) {
            if (content[i] == '<') {
                if (content.substring(i, i + 4) == "<img") {
                    ...
                    var img = content.substring(i, j);
                    if (img.indexOf("<math") > 0) {
                        var i1 = content.indexOf("{", i);
                        var i2 = content.indexOf("}", i1);
                        content = content.substring(0, i1) + content.substring(i2 + 1);
                        found = true;
                    }
                    else {
                        i = j;
                ...
                else if (content.substring(i, i + 5) == "<math") {
    ...
    isPointNearLineSegment: function (x, y, x0, y0, x1, y1, delta = 5) {
        // Tính độ dài đoạn thẳng
        const lineLength = Math.sqrt((x1 - x0) ** 2 + (y1 - y0) ** 2);

        // Nếu đoạn thẳng có độ dài bằng 0 (thực tế là một điểm)
        if (lineLength === 0) {
            // Kiểm tra khoảng cách từ điểm click đến điểm (x0, y0)
            const distanceToPoint = Math.sqrt((x - x0) ** 2 + (y - y0) ** 2);
            return distanceToPoint <= delta;
        }

        // Tính hệ số tham số t cho điểm gần nhất trên đường thẳng
        // Công thức: t = ((x-x0)(x1-x0) + (y-y0)(y1-y0)) / lineLength^2
        const t = ((x - x0) * (x1 - x0) + (y - y0) * (y1 - y0)) / (lineLength ** 2);

        let closestX, closestY;

        // Nếu t < 0, điểm gần nhất là điểm đầu đoạn thẳng
        if (t < 0) {
            closestX = x0;
            closestY = y0;
        }
        // Nếu t > 1, điểm gần nhất là điểm cuối đoạn thẳng
        else if (t > 1) {
            closestX = x1;
            closestY = y1;
        }
        // Nếu 0 <= t <= 1, điểm gần nhất nằm trên đoạn thẳng
        else {
            closestX = x0 + t * (x1 - x0);
            closestY = y0 + t * (y1 - y0);
        }

        // Tính khoảng cách từ điểm click đến điểm gần nhất trên đoạn thẳng
        const distance = Math.sqrt((x - closestX) ** 2 + (y - closestY) ** 2);

        // Kiểm tra xem khoảng cách có nhỏ hơn hoặc bằng delta không
        return distance <= delta;
    },
    isPointInPolygon: function (point, polygon) {
        // Lấy tọa độ x, y của điểm cần kiểm tra
        const x = point.x;
        const y = point.y;
...
const GraphType = { drawing: 0, point: 1, line: 2, rect: 3, circle: 4, oval: 5, image: 6, text: 7, fx: 9, spline: 10, shape: 11, polygon: 12 };
const resizeHandleSize = 8;
const rotationHandleRadius = 12;
...
function Polygon(graphLineStyle = 0, lineWidth = 1) {
    this.type = GraphType.drawing;
    this.data = [];
    this.style = graphLineStyle;
    this.lineWidth = lineWidth;
    ...
    this.draw = function (ctx, centerX, centerY, axis) {
        if (this.data.length > 0) {
            ...
            if (this.isClosed) {
                ctx.lineTo(this.data[0], this.data[1]);
            }
            ctx.lineWidth = this.lineWidth;
            ctx.stroke();
            ctx.setLineDash([0, 0]);
    ...
    this.isInsideShape = function (x, y) {
        let result = mathUtils.isPointInPolygon({ x: x, y: y }, this.getHandles());
        return result;
    }
    ...
    this.moveShape = function (deltaX, deltaY) {
        let index = 0;
        while (index < this.data.length) {
            this.data[index] += deltaX;
            this.data[index+1] += deltaY;
            index += 2;
        }
    }
    ...
    this.getDragOffset = function (offsetX, offsetY) {
        ...
        let index = 0;
        while (index < this.data.length) {
            x = Math.min(this.data[index], x);
            y = Math.min(this.data[index + 1], y);
            index += 2;
        }
        const dragOffsetX = offsetX - x;
        const dragOffsetY = offsetY - y;
    ...
    this.getHandles = function () {
        ...
        let index = 0;
        while (index < this.data.length) {
            handles.push({ x: this.data[index], y: this.data[index + 1] });
            index += 2;
        }
        return handles;
    }
...
function Line(x0 = 0, y0 = 0, x1 = 0, y1 = 0, arrow = 0, graphLineStyle = 0, lineWidth = 1) {
    ...
    this.isInsideShape = function (x, y) {
        return mathUtils.isPointNearLineSegment(x, y, this.data[0], this.data[1], this.data[2], this.data[3], 10);
    }
    this.getCenter = function () {
        return { x: (this.data[0] + this.data[2]) / 2, y: (this.data[1] + this.data[3]) / 2 };
    }
    this.moveShape = function (deltaX, deltaY) {
        this.data[0] += deltaX;
        this.data[2] += deltaX;
    ...
    this.getDragOffset = function (offsetX, offsetY) {
        const dragOffsetX = offsetX - Math.min(this.data[0], this.data[2]);
        const dragOffsetY = offsetY - Math.min(this.data[1], this.data[3]);
        return { x: dragOffsetX, y: dragOffsetY };
    }
...
function Rect(x = 0, y = 0, w = 100, h = 100, graphLineStyle = 0, lineWidth = 1) {
    ...
    this.toData = function () {
        var d = {
            type: this.type,
            data: [this.x, this.y, this.w, this.h, this.rotation],
            style: this.style,
            line: this.lineWidth
        }
        return d;
    ...
    this.getHandles = function () {
        const [left, top, right, bottom] = [this.x, this.y, this.x + this.w, this.y + this.h];
    ...
    this.getRotationHandle = function () {
        ...
        const centerY = (top + bottom) / 2;
        const angle = this.rotation;
        const handleDistance = this.h / 2 + rotationHandleRadius * 2;
    ...
    this.isInsideShape = function (x, y) {
        ...
        const centerX = (left + right) / 2;
        const centerY = (top + bottom) / 2;
        const angle = -this.rotation; // Inverse of the rotation
    ...
    this.getDragOffset = function (offsetX, offsetY) {
        ...
        const rotatedPoint = mathUtils.rotatePoint(offsetX, offsetY, center.x, center.y, -this.rotation);
        const dragOffsetX = rotatedPoint.x - this.x;
        const dragOffsetY = rotatedPoint.y - this.y;
        return { x: dragOffsetX, y: dragOffsetY };
    }
    ...
    this.moveHandle = function (i, x, y) {
        ...
        var point = mathUtils.rotatePoint(x, y, center.x, center.y, -this.rotation);
        x = point.x;
        y = point.y;
        var pos = [this.x, this.y, this.x + this.w, this.y + this.h];
        switch (i) {
            case 0:
                pos[0] = x;
                pos[1] = y;
        ...
        this.x = Math.min(pos[0], pos[2]);
        this.y = Math.min(pos[1], pos[3]);
        this.w = Math.abs(pos[0] - pos[2]);
        this.h = Math.abs(pos[1] - pos[3]);
    }
}
function Circle(x = 0, y = 0, r = 200, graphLineStyle = 0, lineWidth = 1) {
    ...
    this.isInsideShape = function (x, y) {
        ...
        const dx = (x - this.x) / this.r;
        const dy = (y - this.y) / this.r;
    ...
    this.getDragOffset = function (offsetX, offsetY) {
        const dragOffsetX = offsetX - (this.x - this.r);
        const dragOffsetY = offsetY - (this.y - this.r);
        return { x: dragOffsetX, y: dragOffsetY };
    }
    this.moveHandle = function (i, x, y) {
...
function Oval(x = 0, y = 0, rx = 200, ry = 400, graphLineStyle = 0, lineWidth = 1) {
    ...
    this.fromData = function (d) {
        ...
        this.rotation = d.data[4] ? d.data[4] : this.rotation;
        this.style = d.style != null ? d.style : graphLineStyle;
        this.lineWidth = d.line != null ? d.line : 1;
        return this;
    }
    this.toData = function () {
        var d = {
            type: this.type,
            data: [this.x, this.y, this.rx, this.ry, this.rotation],
            style: this.style,
            line: this.lineWidth
        }
        return d;
    ...
    this.getHandles = function () {
        ...
        return [
            mathUtils.rotatePoint(this.x + this.rx, this.y, centerX, centerY, angle),
            mathUtils.rotatePoint(this.x, this.y - this.ry, centerX, centerY, angle)
        ];
    }
    this.isInsideShape = function (x, y) {
        const centerX = this.x;
        ...
        const rotatedPoint = mathUtils.rotatePoint(x, y, centerX, centerY, angle);
        const [rotatedX, rotatedY] = [rotatedPoint.x, rotatedPoint.y];
        const dx = (rotatedX - this.x) / this.rx;
        const dy = (rotatedY - this.y) / this.ry;
        return dx * dx + dy * dy <= 1;
    }
    this.getCenter = function () {
        return { x: this.x, y: this.y };
    ...
    this.getDragOffset = function (offsetX, offsetY) {
        const centerX = this.x;
        const centerY = this.y;
        const rotatedPoint = mathUtils.rotatePoint(offsetX, offsetY, centerX, centerY, -this.rotation);
        const dragOffsetX = rotatedPoint.x - (this.x - this.rx);
        const dragOffsetY = rotatedPoint.y - (this.y - this.ry);
        return { x: dragOffsetX, y: dragOffsetY };
    }
    this.rotateShape = function (angle) {
        this.rotation = angle;
    ...
    this.getRotationHandle = function () {
        ...
        const centerY = this.y;
        const angle = this.rotation;
        const handleDistance = this.ry + rotationHandleRadius * 2;
        return mathUtils.rotatePoint(centerX, centerY - handleDistance, centerX, centerY, angle);
    }
    this.moveHandle = function (i, x, y) {
        var center = this.getCenter();
        ...
        switch (i) {
            case 0:
                this.rx = Math.abs(x - this.x);
                break;
            case 1:
                this.ry = Math.abs(y - this.y);
                break;
        }
    }
...
function GraphText(text = "", x = 0, y = 0,w=0,h=0) {
    ...
    this.text = text;
    this.rotation = 0;
    function drawWrappedText(ctx, text, x, y, maxWidth, maxHeight, rotation, lineHeight) {
        ctx.font = "14px Arial"; // Đặt font TRƯỚC KHI đo đạc
        const fontSize = 14; // Lấy kích thước font (hoặc phân tích từ ctx.font)

        // Ước tính chiều cao dòng nếu không được cung cấp
        if (!lineHeight) {
            // Bạn có thể điều chỉnh hệ số này (ví dụ: 1.2) để tăng/giảm khoảng cách dòng
        ...
        for (let i = 0; i < words.length; i++) {
            const word = words[i];
            // Thử thêm từ mới vào dòng hiện tại (có hoặc không có khoảng trắng)
            const testLine = currentLine === '' ? word : currentLine + ' ' + word;
            const metrics = ctx.measureText(testLine);
            const testWidth = metrics.width;

            // Nếu dòng thử nghiệm vượt quá chiều rộng VÀ dòng hiện tại không rỗng
            if (testWidth > maxWidth && currentLine !== '') {
                lines.push(currentLine); // Lưu lại dòng hiện tại (không bao gồm từ mới)
                currentLine = word;      // Bắt đầu dòng mới với từ hiện tại
            } else {
                // Nếu vừa đủ hoặc đây là từ đầu tiên của dòng
                currentLine = testLine;
            }
        }
        lines.push(currentLine); // Thêm dòng cuối cùng vào danh sách

        // 2. Chuẩn bị vẽ
        ctx.textAlign = "center";   // Căn giữa ngang
        ctx.textBaseline = "middle"; // Căn giữa dọc cho từng dòng

        ctx.save(); // Lưu trạng thái hiện tại (font, màu, transform...)

        // Di chuyển gốc tọa độ đến TÂM của hình chữ nhật để xoay quanh tâm
        ctx.translate(x + maxWidth / 2, y + maxHeight / 2);
        ctx.rotate(rotation); // Xoay

        // 3. Tính toán vị trí bắt đầu vẽ Y để căn giữa toàn bộ khối văn bản
        const totalTextHeight = lines.length * lineHeight;
        // Bắt đầu từ vị trí trên cùng của khối văn bản đã căn giữa
        // Điểm 0,0 bây giờ là tâm xoay. Ta cần dịch lên 1 nửa tổng chiều cao,
        // sau đó dịch xuống 1 nửa chiều cao dòng đầu tiên để căn giữa dòng đầu.
        let currentY = -(totalTextHeight / 2) + (lineHeight / 2);

        // 4. Vẽ từng dòng
        for (let i = 0; i < lines.length; i++) {
            // Kiểm tra tùy chọn: nếu vẽ dòng này sẽ tràn khỏi maxHeight thì dừng lại
            // if (currentY + lineHeight / 2 > maxHeight / 2) {
            //     break;
            // }
            ctx.fillText(lines[i], 0, currentY); // Vẽ tại x=0 (vì đã căn giữa và translate)
            currentY += lineHeight; // Di chuyển xuống dòng tiếp theo
        }

        ctx.restore(); // Khôi phục trạng thái trước khi vẽ
    }
    this.draw = function (ctx, centerX, centerY, axis) {
        drawWrappedText(ctx, this.text, this.x, this.y, this.w, this.h, this.rotation, 16);
    }
    this.fromData = function (d) {
        this.x = d.data[0] ? d.data[0] : 0;
        this.y = d.data[1] ? d.data[1] : 0;
        this.w = d.data[3] ? d.data[3] : 0;
        this.h = d.data[4] ? d.data[4] : 0;
        this.rotation = d.data[5] ? d.data[5] : 0;
    ...
    this.toData = function () {
        var d = {
            type: this.type,
            data: [this.x, this.y, this.text,this.w,this.h,this.rotation]
        }
        return d;
    }
    this.getHandles = function () {
    ...
    this.getRotationHandle = function () {
        ...
        const angle = this.rotation;
        const handleDistance =  this.h / 2 + rotationHandleRadius * 2;
        return mathUtils.rotatePoint(center.x, center.y - handleDistance, center.x, center.y, angle);
    }
    this.isInsideShape = function (x, y) {
        const center = this.getCenter();
        const angle = -this.rotation; // Inverse of the rotation
        const rotatedPoint = mathUtils.rotatePoint(x, y, center.x, center.y, angle);
        const [rotatedX, rotatedY] = [rotatedPoint.x, rotatedPoint.y];
        return rotatedX >= this.x && rotatedX <= this.x+this.w && rotatedY >= this.y && rotatedY <= this.y+this.h;
    }
    this.getCenter = function () {
        const centerX = (this.x +this.w/2) ;
        const centerY = (this.y + this.h/2);
    ...
    this.moveHandle = function (i, x, y) {
        ...
        switch (i) {
            case 0:
                ...
                break;
            case 1:
                this.w = Math.abs(x - this.x);
                this.y = y;
                break;
            case 2:
                this.x = x;
                this.h = Math.abs(y - this.y);
                break;
            case 3:
                this.w = Math.abs(x - this.x);
                this.h = Math.abs(y - this.y);
    ...
    this.setContent = function (initData) {
        ...
        if (initData.objects) {
            ...
            for (var d of initData.objects) {
                ...
                else if (d.type == GraphType.polygon) {
                    data.push(new Polygon().fromData(d));
                }
